...
function clinicalField(page,name){
  ...
  if(!p)return "";
  if(p.type==="multi_select")return multiValue(p).join("、");
  return textValue(p);
}
...
async function downloadClinicalImages(page,propertyName,areaSlug){
  ...
  for(const [index,file] of files.entries()){
    try{
      ...
      const assetPrefix = propertyName.startsWith("生活")
        ? "daily-signs"
        : propertyName.includes("腎機能")
          ? "renal-function"
          : "adr";
      const fileName=`${assetPrefix}-${index+1}.${ext}`;
...
function documentExtension(name,url,contentType=""){
  const fromName=String(name||"").match(/\.(pdf)$/i)?.[1];
  if(fromName)return fromName.toLowerCase();
  try{
    const fromUrl=new URL(url).pathname.match(/\.(pdf)$/i)?.[1];
    if(fromUrl)return fromUrl.toLowerCase();
  }catch{}
  return contentType.split(";")[0].toLowerCase()==="application/pdf"?"pdf":"";
}
async function downloadClinicalDocuments(page,propertyName,areaSlug){
  const files=notionFiles(page,propertyName);
  if(!files.length)return [];
  const assetDir=path.join(OUT,"assets","clinical","therapeutic-areas",areaSlug);
  await fs.mkdir(assetDir,{recursive:true});
  ...
  for(const [index,file] of files.entries()){
    try{
      ...
      if(!response.ok)throw new Error(`HTTP ${response.status}`);
      const contentType=response.headers.get("content-type")||"";
      const ext=documentExtension(file.name,file.url,contentType);
      if(!ext){
        console.warn(`[CLINICAL DOCUMENT] ${propertyName}: PDF以外のファイルをスキップしました (${file.name})`);
        continue;
      }
      const fileName=`renal-function-${index+1}.${ext}`;
      await fs.writeFile(path.join(assetDir,fileName),Buffer.from(await response.arrayBuffer()));
      documents.push({src:`/assets/clinical/therapeutic-areas/${areaSlug}/${fileName}`,name:file.name});
    }catch(error){
      console.warn(`[CLINICAL DOCUMENT] ${propertyName}: ${file.name}を取得できませんでした: ${error?.message||error}`);
    }
  }
  return documents;
}
function clinicalDocumentSection(title,documents){
  if(!documents.length)return "";
  return `<section class="clinical-figure-section"><div class="section-head"><span class="section-bar"></span><h2 class="section-title">${esc(title)}</h2></div>
   <div class="clinical-documents">${documents.map(doc=>`<article class="clinical-document"><iframe src="${esc(doc.src)}#view=FitH" title="${esc(doc.name||title)}" loading="lazy"></iframe><p><a class="more-link" href="${esc(doc.src)}" target="_blank" rel="noopener noreferrer">PDFを開く →</a></p></article>`).join("")}</div></section>`;
}
...
.professional-entry-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-top:28px}
.professional-entry{display:block;text-decoration:none;border:1px solid var(--line);border-radius:15px;padding:22px;background:#fff;box-shadow:var(--shadow)}
.professional-entry small{color:var(--accent);font-weight:800}.professional-entry h2{font-size:21px;margin:8px 0}.professional-entry p{font-size:14px;color:var(--muted);margin:0}
.clinical-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:13px;margin-top:24px}
.clinical-card{display:block;text-decoration:none;border:1px solid var(--line);border-radius:14px;padding:19px;background:#fff}
.clinical-card:hover{border-color:#efad9f}.clinical-card small{color:var(--accent);font-weight:800}.clinical-card h2{font-size:18px;margin:7px 0 4px}.clinical-card p{font-size:13px;color:var(--muted);margin:0}
...
.clinical-figure-section{margin-top:36px}.clinical-figures{display:grid;gap:20px}.clinical-figures figure{margin:0;padding:14px;background:#fff;border:1px solid var(--line);border-radius:18px}.clinical-figures img{display:block;width:100%;height:auto;border-radius:10px}.clinical-documents{display:grid;gap:20px}.clinical-document{margin:0;padding:14px;background:#fff;border:1px solid var(--line);border-radius:18px}.clinical-document iframe{display:block;width:100%;height:720px;border:0;border-radius:10px;background:var(--soft)}.clinical-document p{margin:8px 0 0}
...
<div class="footer-item"><strong><a href="/professionals/">医療・介護職の方へ</a></strong><span>薬効群・観察項目・Q&amp;A</span></div>
...
async function buildFromNotion(){
 ...
 await fs.mkdir(path.join(OUT,"professionals","therapeutic-areas"),{recursive:true});
 await fs.mkdir(path.join(OUT,"professionals","drug-classes"),{recursive:true});
 ...
 for(const area of sortedAreas){
  ...
  const [adrImages,dailySignsImages,renalDocuments]=await Promise.all([
   downloadClinicalImages(area,"ADR（よくある＋要注意）",slug),
   downloadClinicalImages(area,"生活５兆候＋α",slug),
   downloadClinicalDocuments(area,"腎機能",slug)
  ]);
  ...
  await fs.writeFile(path.join(dir,"index.html"),shell(name,`<article class="clinical-page"><div class="kicker">CONDITION / DRUG GROUP</div><h1>${esc(name)}</h1>
   ...
   ${clinicalDocumentSection("腎機能に応じた用量調節",renalDocuments)}
 ...
 for(const c of sortedClinicalClasses){
  ...
  await fs.writeFile(path.join(dir,"index.html"),shell(name,`<article class="clinical-page"><div class="kicker">DRUG CLASS</div><h1>${esc(name)}</h1>
   ...
   <dl class="clinical-data">
    ${clinicalDetailRow("主な薬剤",clinicalField(c,"💊主な薬名"))}
    ${clinicalDetailRow("主な用途",clinicalField(c,"主な用途"))}
    ${clinicalDetailRow("特徴",clinicalField(c,"特徴"))}
    ${clinicalDetailRow("よくみられる症候",clinicalField(c,"☑️ よくみられる症候"))}
    ${clinicalDetailRow("見逃したくない兆候",clinicalField(c,"☑️ 見逃したくない兆候"))}
    ${clinicalDetailRow("食事・嚥下",clinicalField(c,"🍽️ 食事・嚥下"))}
    ${clinicalDetailRow("排泄",clinicalField(c,"🚽 排泄"))}
    ${clinicalDetailRow("睡眠",clinicalField(c,"💤 睡眠"))}
    ${clinicalDetailRow("運動・活動",clinicalField(c,"🏃‍♀️ 運動・活動"))}
    ${clinicalDetailRow("認知機能",clinicalField(c,"🧠 認知機能"))}
    ${clinicalDetailRow("呼吸",clinicalField(c,"呼吸"))}
    ${clinicalDetailRow("循環・転倒",clinicalField(c,"循環・転倒"))}
    ${clinicalDetailRow("主な観察項目",clinicalField(c,"☑️ 主な観察項目"))}
    ${clinicalDetailRow("注意するタイミング",clinicalField(c,"☑️ 注意するタイミング"))}
   </dl><section class="notice"><strong>臨床で使用するときの注意</strong><p>薬剤だけを原因と決めつけず、原疾患、感染、脱水、出血、臓器機能、併用薬なども含めて評価してください。</p></section>
   <p class="clinical-back"><a class="more-link" href="/professionals/drug-classes/">← 薬剤クラス一覧へ戻る</a></p></article>`));
 }
